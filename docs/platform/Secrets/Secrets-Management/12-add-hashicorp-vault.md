...
The Secret ID must not expire and it must be valid until it is manually revoked. Harness uses the App Role ID and Secret ID you supply to fetch a Vault auth token dynamically whenever there is a CRUD operation of secrets related to this vault. For example, when creating a secret in this vault, Harness internally uses the App Role ID and Secret ID and makes a call to Vault via the delegate to generate a token. This token is used to make the actual secret creation call to Vault. Harness does not receive the token. To prevent the generation of a new token for each request (for improved performance), the delegate caches the token for a duration that is one percent less than the token's time to live (TTL). If you want the token to be destroyed after the operation is successfully completed, you can disable token caching.
